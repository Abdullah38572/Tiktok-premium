...
def login():
    """
    Simulates a Termux login page with a randomly generated password.
    """
...
class holo:
    ...
    def main(self):
        try: 
            ...
            print("\n" + self.interface._print("WAIT FOR 🄺🄷🄾🄺🄷🄰🅁  🄱🅁🄰🄽🄳 🅃🄾🄾🄻🅂 IS LOADING......."))
            
            self.driver.get("https://zefoy.com")
            self.wait_for_xpath(self.captcha_box)
            
            print(self.interface._print(" KHOKHAR BRAND TOOLS "))
    ...
    def main_sleep(self, delay):
        while delay != 0:
            time.sleep(1)
            delay -= 1
            self.interface.change_title(f"TikTok 🄺🄷🄾🄺🄷🄰🅁 🄱🅁🄰🄽🄳 🅃🄾🄾🄻🅂 Automator | Cooldown: {delay}s | Github: @ifarra")
    ...
    def wait_for_user(self, xpath):
        print(self.interface._print(" LOADING PLEASE WAIT "))
        while True:
            try:
                time.sleep(3)
                f = self.driver.find_element('xpath', xpath)
